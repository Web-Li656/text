...
import config
from utils import ProcessingError, handle_exception, g_logger
...
class DataCleaner:
    ...
    def process_file(self, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if g_logger:
            g_logger.info(f"文件清洗完成: {file_name}")
    ...
    def process_files(self, file_data_list: list) -> list:
        ...
        for file_data in file_data_list:
            try:
                if not file_data.get('is_valid', True):
                    if g_logger:
                        g_logger.warning(f"跳过无效文件: {file_data.get('file_name')}")
                    continue
                ...
                if g_logger:
                    g_logger.error(f"清洗文件失败 {file_data.get('file_name')}: {str(e)}")
                results.append({
                    'file_name': file_data.get('file_name'),
                    'error': str(e),
                    'cleaned_content': None
        ...
        if g_logger:
            g_logger.info(f"共清洗 {len(results)} 个文件")
