...
class FeatureExtractor:
    ...
    def get_word_frequency(self, text: str, filter_stopwords: bool = False) -> List[Tuple[str, int]]:
        ...
        if utils.g_logger:
            utils.g_logger.debug(f"提取了 {len(most_common)} 个高频词汇")
    ...
    def extract_features(self, text: str) -> Dict[str, Any]:
        ...
        if utils.g_logger:
            utils.g_logger.debug(f"特征提取完成: {char_count} 字符, {word_count} 单词")
    ...
    def process_cleaned_data(self, cleaned_data_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for cleaned_data in cleaned_data_list:
            try:
                ...
                if not text:
                    if utils.g_logger:
                        utils.g_logger.warning(f"文件无有效文本内容: {file_name}")
                    results.append({
                        'file_name': file_name,
                        'features': None,
                        'error': '无有效文本内容'
                ...
                if utils.g_logger:
                    utils.g_logger.info(f"特征提取完成: {file_name}")
                    
            except Exception as e:
                if utils.g_logger:
                    utils.g_logger.error(f"特征提取失败 {cleaned_data.get('file_name')}: {str(e)}")
                results.append({
                    'file_name': cleaned_data.get('file_name', ''),
                    'features': None,
                    'error': str(e)
        ...
        if utils.g_logger:
            utils.g_logger.info(f"共完成 {len(results)} 个文件的特征提取")
