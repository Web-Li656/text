...
class DataReader:
        ...
        try:
            for file_name in os.listdir(self.input_dir):
                ...
                if is_allowed_file_type(file_name):
                    ...
                    if utils.g_logger:
                        utils.g_logger.debug(f"发现有效文件: {file_name}")
        except Exception as e:
            if utils.g_logger:
                utils.g_logger.error(f"读取目录失败: {str(e)}")
            raise ValidationError(f"无法读取输入目录: {str(e)}")
        
        if utils.g_logger:
            utils.g_logger.info(f"共发现 {len(files)} 个有效文件")
    ...
    def validate_utf8_encoding(self, file_path: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            error_msg = "文件编码不是UTF-8"
            if utils.g_logger:
                utils.g_logger.warning(f"{os.path.basename(file_path)}: {error_msg}")
            return False, error_msg
        except Exception as e:
            if utils.g_logger:
                utils.g_logger.error(f"编码验证失败 {file_path}: {str(e)}")
            return False, str(e)
        ...
        try:
            ...
            if isinstance(data, dict):
                if 'text' not in data and 'content' not in data:
                    ...
                    if utils.g_logger:
                        utils.g_logger.warning(f"{file_name}: {warning_msg}")
                    # 不返回False，仅作为警告
            ...
            if utils.g_logger:
                utils.g_logger.error(f"{file_name}: {error_msg}")
    ...
    def read_file(self, file_name: str) -> Dict[str, Any]:
        ...
        if check_readonly_file(file_path):
            if utils.g_logger:
                utils.g_logger.info(f"检测到只读参考文件: {file_name}")
        ...
        if utils.g_logger:
            utils.g_logger.info(f"文件读取成功: {file_name}")
    ...
    def read_all_files(self) -> List[Dict[str, Any]]:
        ...
        for file_name in file_names:
            try:
                ...
                if utils.g_logger:
                    utils.g_logger.error(f"读取文件失败 {file_name}: {str(e)}")
                results.append({
                    'file_name': file_name,
                    'file_path': os.path.join(self.input_dir, file_name),
                    'content': None,
    ...
    def read_readonly_reference(self) -> Optional[str]:
        ...
        if not os.path.exists(config.READONLY_REFERENCE_FILE):
            if utils.g_logger:
                utils.g_logger.warning(f"只读参考文件不存在: {config.READONLY_REFERENCE_FILE}")
            return None
        ...
        try:
            with open(config.READONLY_REFERENCE_FILE, 'r', encoding='utf-8') as f:
                content = f.read()
            if utils.g_logger:
                utils.g_logger.info(f"成功读取只读参考文件: {config.READONLY_REFERENCE_FILE}")
            return content
        except Exception as e:
            if utils.g_logger:
                utils.g_logger.error(f"读取只读参考文件失败: {str(e)}")
            return None
