...
import config
from utils import validate_relative_path, ProcessingError, handle_exception, g_logger
...
class ReportGenerator:
    ...
    def save_report(self, file_name: str, content: str) -> str:
        ...
        if g_logger:
            g_logger.info(f"报告已保存: {report_path}")
    ...
    def generate_and_save_report(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if g_logger:
            g_logger.info(f"报告生成完成 [{status}]: {report_name}")
    ...
    def generate_batch_reports(self, analysis_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for result in analysis_results:
            try:
                ...
                if g_logger:
                    g_logger.error(f"生成报告失败 {result.get('file_name')}: {str(e)}")
                reports.append({
        ...
        if g_logger:
            g_logger.info(f"共生成 {len(reports)} 份报告")
...
def generate_summary_report(
    ...
    if g_logger:
        g_logger.info(f"汇总报告已保存: {output_path}")
